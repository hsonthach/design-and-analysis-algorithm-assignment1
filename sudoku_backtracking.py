...
def find_empty_location(grid: Matrix) -> Optional[Tuple[int, int]]:
    ...
    With priority from lower row & lower cell 
    """
    for i in range(9):
        for j in range(9):
            if grid[i][j] == 0:
...
def sudoku(grid: Matrix) -> Optional[Matrix]:
    ...
    # Happen when no digit (1->9) can be placed at that row,column
    # So it need to tell parent that if the parent's digit is not changed, there would be no solution
    # -> Cleaning up grid[row][column] and tell the parent (if any) that parent's digit is not ok
    grid[row][column] = 0
    return None
